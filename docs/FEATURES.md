...
>
> Margin behind an open position is **reserved in the vault**, not just in the ledger: the keeper locks it before an order is confirmed, so `withdraw()` reverts above `balance − locked`. Verified live on Coston2.
