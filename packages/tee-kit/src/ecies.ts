/**
 * ECIES sealing — client → enclave confidentiality.
 *
 * A client encrypts a payload to the enclave's public key. Only the process
 * holding the matching private key can read it, so the API tier in between
 * relays ciphertext it cannot open: the payload is confidential from your own
 * backend, not merely from the public.
 *
 * Scheme (secp256k1 ECIES, the curve Flare and Ethereum already use):
 *   ephemeral keypair → ECDH with the enclave pubkey → HKDF-SHA256 → AES-256-GCM
 *   wire: ephemeralPubKey(65) | iv(12) | ciphertext+tag
 *
 * Uses @noble/curves for secp256k1 — the same audited library viem depends on,
 * so there is no version skew — and Node's built-in crypto for HKDF and AES-GCM.
 *
 * Worth pairing with a timelock (drand, say) when the requirement is not only
 * "nobody outside the enclave can read this" but "nobody can read it *yet*":
 *   • timelock  → unreadable by anyone, including the enclave, until a round lands
 *   • ECIES     → readable only by attested enclave code, ever
 * Together they cover both the "before" and the "who".
 */
